...
public interface IdriRepostory extends JpaRepository<IdriBean,String> {
    ...
    /*根据日期和加权类型查询数据*/
    @Query(value="select  i.indu_code, i.index_date,i.idri ,i.weight_type ,i.corp_count from idri_20171025 i where i.index_date>=?1 and i.index_date<=?2 and i.weight_type=?3 and i.indu_code IN('B','C','D','E','F','G','I','K')  ORDER BY i.indu_code",nativeQuery = true)
